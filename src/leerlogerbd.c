...
*	Version 1.3 (28/10/2011) fallo medianoche, se envian dato sdia siguiente (case 'F')
*/
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
...
QM QmHis[NUMHISTQM];
int ReadLogerBd();
static char *version = "1.3 (28/10/2011)";
...
int argc;
{
	...
	if (argc==2){
		if((argv[1][0]!='R') && (argv[1][0]!='C') && (argv[1][0]!='U') && (argv[1][0]!='L') && (argv[1][0]!='T') && (argv[1][0]!='F') && (argv[1][0]!='S') ){
			printf("\n\tSAIHBD Parametros: leerlogerbd R|C|U|L|T|F|S\n");
			exit(0);}
	}
	...
	case 'I':
		...
		//
			printf("<a id=\"fijarCabecera\" class=\"button\" href=\"#\">Fijar Encabezado</a>");
			printf("<a id=\"quitarCabecera\" class=\"button\" href=\"#\">Eliminar Encabezado Fijo</a>");
			printf("\n<div id=\"tablaDIA\" style=\"overflow:auto; height:450px;width:760px;\">");
		//	printf("\n<table id=\"tablaDIA\" class=\"datos\">");
		...
		case 'F':	//salida fichero SAICA
			...
			// si estamos en el entorno de la medianoche no se genera el fichero
			if ( (numhistqm > 1419) ||  (numhistqm < 31) ) break;	
